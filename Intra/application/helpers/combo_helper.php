...
function comboPalavrasAprovadas($name,$javascript, $selected,$param=null){
    ...
    $sql = "select 	
                    distinct
                        p.palavra_id
                       ,p.palavra 
               from acervo.palavra p
                       inner join acervo.obra_palavra op
                               on p.palavra_id = op.palavra_id
                       inner join acervo.obra o
                               on o.obra_id = op.obra_id
               where o.status_id = 2 and flag_aprovacao_autor = 1
               order by p.palavra";
    //echo $sql;
    $data = $ci->db->query($sql)->result();
    ...
    foreach ($data as $d){      
      $cmb .= "<option value='".$d->palavra_id."'";
        $cmb .= $selected==$d->palavra_id?" selected='selected'":"";
        $cmb .= ">". rupper($d->palavra).'</option>';
    }   
    $cmb .="</select>";
    return $cmb;
}
