...
foreach ($non_pk as $row) {
    ...
    if ($row["data_type"] == 'text' or (int)$row['character_maximum_length'] >=400) {
        ...
        <div class=\"form-group\">
            <textarea class=\"form-control\" rows=\"3\" name=\"" . $row["column_name"] . "\" id=\"" . $row["column_name"] . "\" $required  placeholder=\"" . label($row["column_name"]) . "\"  ><?php echo $" . $row["column_name"] . "; ?></textarea>
        </div>
            </td>";
    } else {
