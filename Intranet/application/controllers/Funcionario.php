...
class Funcionario extends CI_Controller
{
	...
	public function index()
	{
		...
		$param_est_organizacional_id = (int)($this->input->get('est_organizacional_id', TRUE));
		$start = intval($this->input->get('start'));

		$param = ' est_organizacional_st = 0 and est_organizacional_id not in (178,185,259)';
		$param .= ' and est_organizacional_id in (select distinct est_organizacional_id from vi_pessoa )';
		$est_organizacional = $this->Est_organizacional_model->get_all_combobox($param, 'upper(est_organizacional_sigla)');
		...
		$config['total_rows'] = 0; //$this->Funcionario_model->total_rows($q);

		$param = ' and f.funcionario_tipo_id in (11,1,7,2,3) ';
		if (!empty($q)) {
			$param .= "
							and (
								v.pessoa_nm ilike '%$q%'
								or v.est_organizacional_sigla ilike '%$q%'
							)
		
					";
		}
		if (!empty($param_est_organizacional_id)) {
			$param .= "and v.est_organizacional_id = " . $param_est_organizacional_id;
		}


		if (empty($q) and empty($param_est_organizacional_id)) {
			$param =  " and 1=2 ";
		}


		$funcionario = $this->Funcionario_model->get($param);
		...
		$data = array(
			'titulo' => 'Todos colaboradores Colaboradores - Eventual e Cargo',
			'funcionario_data' => json($funcionario),
			'est_organizacional' => json($est_organizacional),
			'q' => $q,
			'format' => $format,
	...
	public function listar_todos()
	{
		...
		$param = ' est_organizacional_st = 0 and est_organizacional_id not in (178,185,259)';
		// $param .= ' and est_organizacional_id in (select distinct est_organizacional_id from vi_pessoa_todos )';
		$est_organizacional = $this->Est_organizacional_model->get_all_combobox($param, 'upper(est_organizacional_sigla)');
		...
		$param = ' and f.funcionario_tipo_id in (11,1,7,2,12,3,10,14) ';
		if (!empty($q)) {
			$q = remove_acento(utf8_decode(rupper($q)));
			$param .= "
							and (
								remove_acento(upper(v.pessoa_nm)) ilike remove_acento('%$q%')
								or v.est_organizacional_sigla ilike '%$q%'
							)
...
/* Gerado por RGenerator - 2024-01-23 21:14:10 */