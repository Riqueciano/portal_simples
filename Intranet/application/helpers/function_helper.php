...
{
    // Mapeamento de caracteres com acentos para os sem acentos
    ...
    // Converter qualquer outro caractere especial para ASCII
    $string = iconv('UTF-8', 'ASCII//TRANSLIT//IGNORE', $string);

    return $string;
}

function associativeArrayToString($array, $separator = '||')
{
    $string = '';
    foreach ($array as $key => $value) {
        $string .= $key . '=' . $value . $separator;
    }
    // Remover o separador final
    return rtrim($string, $separator);
}

function stringToAssociativeArray($string, $separator = '||')
{
    $array = [];
...
function objectToArray($obj)
{
    ...
    // Se é um array, percorre cada elemento
    if (is_array($obj)) {
        $newArray = [];
        foreach ($obj as $key => $val) {
            // Converte cada valor do array recursivamente
            $newArray[$key] = objectToArray($val);
        }
    } else {
        // Se não é um objeto nem um array, retorna o valor
        $newArray = $obj;
    }
...
function VALIDA_ORIGEM_REQUEST_old()
{  
    $allowed_origin = $_SERVER['HTTP_HOST'] ?? '';  // Recupera o host da requisição
    $origin = $_SERVER['HTTP_ORIGIN'] ?? '';        // Verifica a origem da requisição
    $referer = $_SERVER['HTTP_REFERER'] ?? '';      // Verifica o referer da requisição
    $user_agent = $_SERVER['HTTP_USER_AGENT'] ?? '';  // Verifica o user agent
    $sec_fetch = $_SERVER['HTTP_SEC_FETCH_SITE'] ?? '';  // Verifica o cabeçalho Sec-Fetch-Site

    // Verifica se a requisição não está vindo do Postman ou de ferramentas semelhantes
    if (strpos($user_agent, 'PostmanRuntime') !== false || empty($sec_fetch)) {
        echo 'Requisição não permitida';
        exit;
    }

    // Libera se for do Google, Bing ou Yahoo
    if ($referer && (strpos(parse_url($referer, PHP_URL_HOST), 'google.com') !== false || 
                     strpos(parse_url($referer, PHP_URL_HOST), 'bing.com') !== false ||
                     strpos(parse_url($referer, PHP_URL_HOST), 'yahoo.com') !== false)) {
        return;
    }

    // Permite requisições somente do localhost
    if (empty($origin) && empty($referer) && $allowed_origin === 'localhost') {
        return;
    }

    // Verifica se o Origin está presente e é válido
    if ($origin && parse_url($origin, PHP_URL_HOST) !== $allowed_origin) {
        echo 'Requisição não permitida';
        exit;
    }

    // Só valida o referer se ele estiver presente
    if ($referer && parse_url($referer, PHP_URL_HOST) !== $allowed_origin) {
        echo 'Requisição não permitida';
        exit;
    }

    // Flexibiliza para produção, permitindo requisições legítimas de localhost ou do site portalsdr.ba.gov.br
    if (!$origin && !$referer && ($allowed_origin === 'localhost' || $allowed_origin === 'portalsdr.ba.gov.br')) {
        error_log('Aviso: Requisição sem Origin ou Referer detectada, mas proveniente de host autorizado. Prosseguindo com cautela.');
    }
}
...
function DBToData($data)
{
    $data = trim($data);
    return str_replace('//', '', $data[8] . $data[9] . "/" . $data[5] . $data[6] . "/" . $data[0] . $data[1] . $data[2] . $data[3]);
}
