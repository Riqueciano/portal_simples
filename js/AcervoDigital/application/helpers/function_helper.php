...
function valida_login_campanha(){
    if(empty($_SESSION['comprador_id'])){
        echo "Sessão Encerrada, <b><a href='".site_url('comprador/login')."'>CLIQUE AQUI</a></b> para entrar novamente";exit;
    }
}
...
function retornaPosicaoCronograma($chamada_publica_id=0, $dt_ref){
    $ci = get_instance();
    $query = "select * from sigater_direta.chamada_publica where chamada_publica_id = $chamada_publica_id";
