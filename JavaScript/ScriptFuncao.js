...
function mascaraTelefone(evento, objeto){
    ...
    //CAPTURA A QUANTIDADE DE CARACTERES DO CAMPO
    var qtd_caracteres = campo.value.length;    
    
    //VERIFICA SE A QUANTIDADE DE NÚMEROS É ATÉ 8 (1234-5678)
    if ((caracteres.search(String.fromCharCode (keypress))!=-1) && qtd_caracteres <= 8)
    {   
        //VERIFICA SE É O 4° NÚMERO E INSERE O SEPARADOR
        if (campo.value.length == 4 ){campo.value = campo.value + separacao1;}
    }
    //SE A QUANTIDADE DE NÚMEROS FOR 9 (91234-56789)
    else if ((caracteres.search(String.fromCharCode (keypress))!=-1) && qtd_caracteres == 9)
        {
            //SEPARA OS NÚMEROS REMOVENDO O SEPARADOR
            var num = campo.value.split('-');
            
            //REORDENA A SEPARADOR PARA O PADRÃO COM 9 DÍGITOS
            campo.value = num[0] + num[1].substring(0,1) + separacao1 + num[1].substring(1,5);
        }
        else{event.returnValue = false;}
}
/*******************************************************************************
