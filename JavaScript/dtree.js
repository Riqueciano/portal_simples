...
// ----------------------------------------------------------------------------
	...
	this.imgFolder		= 'https://www.portalsema.ba.gov.br/_portal/Imagens/Dtree/';
	this.target				= null;
	this.hasLines			= true;
			...
			if (this.arrNodes[n].pid == pNode)
			{
				...
				if (this.rootNode != cn.pid)
				{
					...
					for (r=0; r<this.arrRecursed.length; r++)
						this.strOutput += '<img  border="0" src="' + this.imgFolder + ( (this.arrRecursed[r] == 1 && this.hasLines) ? 'line' : 'empty' ) + '.gif" alt="" />';
