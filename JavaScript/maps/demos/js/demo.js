	...
	window.demo = { 
		'version': '3.0-rc1',
		'ga': '',
		'primaryUrl': 'https://code.google.com/p/jquery-ui-map/',
		'url': 'https://jquery-ui-map.googlecode.com/', 
		'forum': 'https://groups.google.com/group/jquery-ui-map-discuss/feed/rss_v2_0_msgs.xml', 
		'subscribe': 'https://groups.google.com/group/jquery-ui-map-discuss/boxsubscribe', 
		...
		'init': function() {
			...
			//Modernizr.load({ 'test': ( location.href.indexOf(this.url) > -1 ), 'yep': 'https://www.google-analytics.com/ga.js' });
			this.test('Backbone', function() {
				...
				ForumCollection = Backbone.Collection.extend({ 'url': 'https://ajax.googleapis.com/ajax/services/feed/load?v=1.0&callback=?&q={0}'.format(encodeURIComponent(demo.forum)), 'parse': function(response) { return response.responseData.feed.entries; } });
