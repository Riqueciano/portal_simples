...
class Spreadsheet_Excel_Reader {
    ...
    // DUMP AN HTML TABLE OF THE ENTIRE XLS DATA
    // =========================================
    function retornaInsertsDotacaoOrcamentaria($arquivo_base_id = 0, $sheet = 0, $table_class = 'excel') {

        //$row = 2 para NÃO pegar o titulo das colunas
        for ($row = 2; $row <= $this->rowcount($sheet); $row++) {
            $rowheight = $this->rowheight($row, $sheet);

            $out .="insert into orcamento.dotacao_orcamentaria ( 
                                                 exercicio 
                                                ,poder_cod 
                                                ,poder_nm 
                                                ,orgao_cod 
                                                ...
                                                ,programa_nm 
                                                ,programa_sigla 
                                                /*,estrutura_estrategica_cod 
                                                ,estrutura_estrategica_nm */
                                                ,produto_cod 
                                                ,produto_nm 
                                                ,produto_sigla 
                                                ,unidade_medida_cod 
    ...
    function criaIndice($row_numbers = false, $col_letters = false, $sheet = 0, $table_class = 'excel') {
        ...
        for ($row = 1; $row <= $this->rowcount($sheet); $row++) {
            ...
            //varre as colunas
            for ($col = 1; $col <= $this->colcount($sheet); $col++) {
                #1#pega o item a ser criado
                if ($row == 1 and $col == 1) {
                    $item = $this->val($row, $col, $sheet);
                    //valida se o item ja existe
                    $sql = " select item_id from indice.item where item_nm ilike '%$item%' limit 1";
                    $rs = pg_query(abreConexao(), $sql);
                    $ultimoValorSeq = pg_fetch_assoc($rs);
                    $item_id = $ultimoValorSeq['item_id'];
                    if (pg_num_rows($rs) > 0) {
                        //se existe atualiza
                    } else {//se nao insere
                        $sql = "insert into indice.item(item_nm) values ('$item') RETURNING item_id";
                        $rs = pg_query(abreConexao(), $sql);
                        $ultimoValorSeq = pg_fetch_assoc($rs);
                        $item_id = $ultimoValorSeq['item_id'];
                    }
                ...
                ##PEGA O ANO
                $ano = $this->val(3, $this->colcount($sheet), $sheet);

                #2###INSERE OS INDICES
                if ($row == 2 and $col != 1) {

                    $indice = $this->val($row, $col, $sheet);
                    //nao pega a primeira e ultima coluna
                    if (rupper($indice) != rupper('Município') and rupper($indice) != rupper('Ano')) {


                        //valida se o item ja existe
                        $sql = " select indice_id from indice.indice where indice_nm ilike '%" . rupper($indice) . "%' and ano = $ano";
                        echo $sql . '<br>';
                        $rs = pg_query(abreConexao(), $sql);
                        $ultimoValorSeq = pg_fetch_assoc($rs);
                        if (pg_num_rows($rs) > 0) {
                            //se existe atualiza
                            $indice_id[] = $ultimoValorSeq['indice_id'];
                        } else {//se nao insere
                            $sql = "insert into indice.indice(indice_nm, item_id, ano) values ('" . rupper($indice) . "', '$item_id', $ano) RETURNING indice_id";
                            $rs = pg_query(abreConexao(), $sql);
                            $ultimoValorSeq = pg_fetch_assoc($rs);
                            $indice_id[] = $ultimoValorSeq['indice_id'];
                        }
            ...
            // echo $indice_id[2];exit;
            //varre novamente  colunas
            for ($col = 1; $col <= $this->colcount($sheet); $col++) {

                if ($row > 2 and $col > 1 and $col != $this->colcount($sheet)/* ultima coluna */) {


                    $municipio = $this->val($row, 1, $sheet);
                    $indice = $this->val($row, $col, $sheet);
                    $indiceExibir = $indice;
                    if (empty($indice)) {
                        $indiceExibir = 0;
                    }
                    //valida se registro ja existe
                    //lower(to_ascii(nomemunic)) like lower(to_ascii('%PáuLô%'))
                    $sqlV = "select 1 from indice.indice_municipio where  indice_id = " . $indice_id[$cont] . "
                                       and municipio_id = (select municipio_id 
                                       from indice.municipio where insensitive(municipio_nm) ilike insensitive('" . $municipio . "'))
                                           and indice_municipio_valor = " . $indiceExibir . "
                                
                                ";

                    ECHO $sqlV . '<br>'; //EXIT;
                    $rs0 = pg_query(abreConexao(), $sqlV);
                    if (pg_num_rows($rs0) == 0) {



                        $sql = "insert into 
                                 indice.indice_municipio
                                 (
                                     indice_id
                                    ,municipio_id
                                    ,indice_municipio_valor
                                    ) values(" . $indice_id[$cont] . " 
                                    ,(select municipio_id from indice.municipio where insensitive(municipio_nm) ilike insensitive('" . (rlower($municipio)) . "')    )
                                    ," . $indiceExibir . " )";
    ...
    function criaIndiceFast($row_numbers = false, $col_letters = false, $sheet = 0, $table_class = 'excel') {
    ...
    function i_dapsMunicipio($row_numbers = false, $col_letters = false, $sheet = 0, $table_class = 'excel') {
        ...
        for ($row = 1; $row <= $this->rowcount($sheet); $row++) {
            ...
            $c = 0;
            for ($col = 1; $col <= $this->colcount($sheet); $col++) {
                //so inserir apartir da linha 3
                if ($row > 2 and $col > 1 and $col != $this->colcount($sheet)/* ultima coluna */) {

                    $municipio = str_replace("'", "", $this->val($row, 1, $sheet));
                    $dapsAtivas = str_replace("-", "", str_replace("?", "", str_replace("*", "", str_replace(" ", "", str_replace(",", "", $this->val($row, 2, $sheet))))));
                    $dapsInativas = str_replace("-", "", str_replace("?", "", str_replace("*", "", str_replace(" ", "", str_replace(",", "", $this->val($row, 3, $sheet))))));
                    $ano = (int) $this->val($row, 4, $sheet);

                    $sql = "insert into 
                                 indice.i_daps_municipio
                                 (
                                     municipio_id
                                    ,daps_ativa
                                    ,daps_inativa
                                    ...
                                    values (
                                    (select municipio_id from indice.municipio where insensitive(municipio_nm) ilike insensitive('" . trim((rlower($municipio))) . "')    )
                                    ,'" . (int)$dapsAtivas . "' 
                                    ,'" . (int)$dapsInativas . "' 
                                    ,'" . $ano . "' 
                                  )";
    ...
    function i_estabelecimentoAgroPecoMunicipio($row_numbers = false, $col_letters = false, $sheet = 0, $table_class = 'excel') {
        ...
        for ($row = 1; $row <= $this->rowcount($sheet); $row++) {
            ...
            for ($col = 1; $col <= $this->colcount($sheet); $col++) {
                ...
                $municipio = str_replace("'", "", trim($this->val($row, 1, $sheet)));
                $estabelecimentoQtd = str_replace("-", "", str_replace("?", "", str_replace("*", "", str_replace(" ", "", str_replace(",", "", $this->val($row, 2, $sheet))))));
                $ano = (int) $this->val($row, 3, $sheet);

                    $sql = "insert into 
                                 indice.i_estabelecimento_agropeco_municipio 
                                 (
                                     municipio_id
                                    ,estabelecimento_qtd
                                    , ano
    ...
    function i_pibMunicipalSetor($row_numbers = false, $col_letters = false, $sheet = 0, $table_class = 'excel') {
        ...
        for ($row = 1; $row <= $this->rowcount($sheet); $row++) {
            ...
            for ($col = 1; $col <= $this->colcount($sheet); $col++) {
                ...
                $municipio              = str_replace("'", "", trim($this->val($row, 1, $sheet)));
                $pibAgropecuaria        = str_replace("-", "", str_replace("?", "", str_replace("*", "", str_replace(" ", "", str_replace(",", ".", $this->val($row, 2, $sheet))))));
                $pibIndustria           = str_replace("-", "", str_replace("?", "", str_replace("*", "", str_replace(" ", "", str_replace(",", ".", $this->val($row,3, $sheet))))));
                $pibServico             = str_replace("-", "", str_replace("?", "", str_replace("*", "", str_replace(" ", "", str_replace(",", ".", $this->val($row, 4, $sheet))))));
                $ano                    = (int) $this->val($row, 5, $sheet);

                    $sql = "insert into 
                                 indice.i_pib_Municipal_Setor 
                                 (
                                      municipio_id
                                    , pib_agropecuaria
                                    , pib_industria
    ...
    function i_populacaoMunicipalFaixaEtaria($row_numbers = false, $col_letters = false, $sheet = 0, $table_class = 'excel') {
        ...
        for ($row = 1; $row <= $this->rowcount($sheet); $row++) {
            ...
            for ($col = 1; $col <= $this->colcount($sheet); $col++) {
                ...
                $municipio              = str_replace("'", "", trim($this->val($row, 1, $sheet)));
                $situacaoDomicilio      = trim($this->val($row, 2, $sheet));
                $faixaEtaria            = trim($this->val($row, 3, $sheet));
                $total                  = str_replace("-", "", str_replace("?", "", str_replace("*", "", str_replace(" ", "", str_replace(",", ".", $this->val($row,4, $sheet))))));
                $ano                    = (int) $this->val($row, 5, $sheet);

                    $sql = "insert into 
                                 indice.i_populacao_municipio_faixaetaria
                                 (
                                      municipio_id
                                    , situacao_domicilio
                                    , faixa_etaria
    ...
    //População Censitária por Município e Idade2
    function i_populacaoMunicipioIdade($row_numbers = false, $col_letters = false, $sheet = 0, $table_class = 'excel') {
        ...
        for ($row = 1; $row <= $this->rowcount($sheet); $row++) {
            ...
            for ($col = 1; $col <= $this->colcount($sheet); $col++) {
                ...
                $ano                    = (int) $this->val($row, 5, $sheet);
                    ...
                    $sql = "insert into 
                                 indice.i_populacao_municipio_idade
                                 (
                                      municipio_id
                                    , situacao_domicilio
                                    , faixa_etaria
    ...
    //População Censitária por Município e Idade2
    function i_populacaoEstimadaMunicipio($row_numbers = false, $col_letters = false, $sheet = 0, $table_class = 'excel') {
        ...
        for ($row = 1; $row <= $this->rowcount($sheet); $row++) {
            ...
            for ($col = 1; $col <= $this->colcount($sheet); $col++) {
                ...
                $municipio              = str_replace("'", "", trim($this->val($row, 1, $sheet)));
                $quantPessoas          = (int)trim($this->val($row, 2, $sheet));
                 $ano                    = (int) $this->val($row, 3, $sheet);

                    $sql = "insert into 
                                 indice.i_populacao_estimada_municipio
                                 (
                                      municipio_id
                                    , quant_pessoas
                                    , ano
    ...
    //População Censitária por Município e Idade2
    function i_populacaoMunicioIdadeSexo($row_numbers = false, $col_letters = false, $sheet = 0, $table_class = 'excel') {
        ...
        for ($row = 1; $row <= $this->rowcount($sheet); $row++) {
            ...
            for ($col = 1; $col <= $this->colcount($sheet); $col++) {
                ...
                $municipio              = str_replace("'", "", trim($this->val($row, 1, $sheet)));
                $situacao_domicilio     = trim($this->val($row, 2, $sheet));
                $faixa_etaria           = trim($this->val($row, 3, $sheet));
                $homens                 = (int)trim($this->val($row, 4, $sheet));
                $mulheres               = (int)trim($this->val($row, 5, $sheet));
                $ano                    = (int) $this->val($row, 6, $sheet);


                 
                    $sql = "insert into 
                                 indice.i_populacao_municipio_idade_sexo
                                 (
                                      municipio_id
                                    , situacao_domicilio
                                    , faixa_etaria
    ...
    function i_populacaoMunicipioFaixaEtariaSexo($row_numbers = false, $col_letters = false, $sheet = 0, $table_class = 'excel') {
        ...
        for ($row = 1; $row <= $this->rowcount($sheet); $row++) {
            ...
            for ($col = 1; $col <= $this->colcount($sheet); $col++) {
                ...
                $ano                    = (int) $this->val($row, 6, $sheet);
                    ...
                    $sql = "insert into 
                                 indice.i_populacao_municipio_faixaetaria_sexo
                                 (
                                      municipio_id
                                    , situacao_domicilio
                                    , faixa_etaria
