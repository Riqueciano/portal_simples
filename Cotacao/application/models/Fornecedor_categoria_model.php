...
class Fornecedor_categoria_model extends CI_Model
{

    public $table = 'cotacao.fornecedor_categoria';
    public $id = 'fornecedor_categoria_id';
    public $order = 'DESC';
    ...
    {
        $this->db->select("$this->id as id, fornecedor_categoria_nm as text");
        if(!empty($param)){
            $this->db->where($param);
        }
        if(!empty($order)){
    ...
    { 
        ...
        if ($q != null) {
        $this->db->ilike('cotacao.fornecedor_categoria.fornecedor_categoria_id', $q);
	$this->db->or_ilike('fornecedor_categoria.fornecedor_categoria_nm', $q);
       }//end if
    ...
    function get_limit_data($limit, $start = 0, $q = NULL) {
        $this->db->select('*');
        $this->db->order_by($this->table.'.'.$this->id, $this->order);
        $this->db->ilike('fornecedor_categoria.fornecedor_categoria_id', $q);
	$this->db->or_ilike('fornecedor_categoria.fornecedor_categoria_nm', $q);
	$this->db->limit($limit, $start);
        return $this->db->get($this->table)->result();
    }
...
/* Final do arquivo Fornecedor_categoria_model.php */
/* Local: ./application/models/Fornecedor_categoria_model.php */
/* Data - 2024-06-17 20:13:42 */