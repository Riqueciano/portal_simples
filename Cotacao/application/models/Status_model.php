...
class Status_model extends CI_Model
{
    ...
    {
        $this->db->select("$this->id as id, status_nm as text");
        if(!empty($param)){
            $this->db->where($param);
        }
        if(!empty($order)){
    ...
    function total_rows($q = NULL) {
        ...
        $this->db->ilike('cotacao.status.status_id', $q);
	$this->db->or_ilike('status.status_nm', $q);
    ...
    function get_limit_data($limit, $start = 0, $q = NULL) {
        $this->db->select('*');
        $this->db->order_by($this->table.'.'.$this->id, $this->order);
        $this->db->ilike('status.status_id', $q);
	$this->db->or_ilike('status.status_nm', $q);
...
/* Final do arquivo Status_model.php */
/* Local: ./application/models/Status_model.php */
/* Data - 2022-09-05 18:17:33 */